...
import { Text, TextInput, StyleSheet, View, TouchableOpacity, Image, Animated, Dimensions } from 'react-native';
...
// 날씨 데이터
const API_KEY1 = "0ca7181915af9ce98c71cd28a0dce50c";
// 미세먼지 데이터
const API_KEY2 = 'QzUQJ513oqNAs5HYawd6uiT1ve0tFj3hyiMv7VA%2BsycoFnNnTd6Yjwdz6tbiVpFNMt8cULZQo9a4DOtQey8hiQ%3D%3D';
...
export default function Main({ navigation }) {
  ...
  // 즐겨찾기 추가/삭제 함수
  const isFavorite = (marker) => favorites.some((fav) => fav.pfctSn === marker.pfctSn);
  ...
  const ask = async () => {
    ...
    //날씨
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&lang=kr&units=metric&appid=${API_KEY1}`)
    const json = await response.json();
    //console.log(json);
    

    // sidoName이 서울인데 인코딩해서 넣어야 함 -> %EC%84%9C%EC%9A%B8
    const dust = await fetch(`http://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getCtprvnRltmMesureDnsty?serviceKey=${API_KEY2}&sidoName=%EC%84%9C%EC%9A%B8&returnType=json`)
...
const fetchProtectMarkers = async () => {
  ...
  const handleSearch = () => {
    const foundMarker = markers.find(marker => marker.pfctNm.includes(searchQuery));
     ...
     <TouchableOpacity
      ...
      >
            ...
            return (
              <Marker
                ...
                onPress={() => {
                  ...
                  >
                    <Callout style={{flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingLeft: 12, paddingTop: 6}}>
                      
                        <Text style={{ textAlign: 'center', fontWeight: 'bold' }}>{play.pfctNm}</Text>
                        <Text style={{ textAlign: 'center' }}>{firstPart.trim()}</Text> 
                        <Text style={{ textAlign: 'center' }}>{secondPart?.trim()}</Text> 
        ...
        {policeMarkers.map((police, index) => (
          ...
          >
            <Callout>
              <Text style={{textAlign: 'center'}}>{`${police.name1} ${police.name2} ${police.name3}`}</Text>
            </Callout>
          </Marker>
        ))}
        {/* 어린이 보호구역 원 표시 */}
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity onPress={moveToCurrentLocation} style={styles.iconButton}>
          <Image source={require('../assets/location.png')} style={styles.icon} />
        </TouchableOpacity>
      </View>

      {weatherData && (
        <TouchableOpacity onPress={() => navigation.navigate('WeatherPage', {weatherData})} style={{ flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: 'white', position: 'absolute', bottom: 98, left: 15, width: 44, height: 80, borderRadius: 30, shadowColor: '#000',
          shadowOffset: { width: 0, height: 2 },
          shadowOpacity: 0.3,
          shadowRadius: 2,
          elevation: 5 }}>
          <Image source={{ uri: weatherData.weatherIconUrl }} style={{ width: 50, height: 50, position: 'absolute', bottom: 35}} />
          <Text style={{fontSize: 16, textAlign: 'center', marginLeft: 3, marginTop: 30 }}>{weatherData.temp}°</Text>
          {/* 게이지바 */}
          <View style={styles.gaugeContainer}>
            <View style={[styles.gaugeFill, { width: `${weatherData.dust10}%`, backgroundColor: getGaugeColor(weatherData.dust10) }]} />
          </View>
          <Text style={{fontSize: 9, color: getGaugeColor(weatherData.dust10), fontWeight: 'bold'}}>미세</Text>
        </TouchableOpacity>
      )}

      <View style={{position: 'absolute', bottom: 40, right: 15}}>
        <TouchableOpacity style={styles.videoButton} onPress={() => navigation.navigate('Chat')}>
          <Image source={require('../assets/chatbot.png')} style={[styles.videoIcon]} />
        </TouchableOpacity>
      </View>

      <View style={{position: 'absolute', top: 130, left: 35}}>
        <TouchableOpacity style={styles.rewardBox}>
          <Image source={require('../assets/reward.png')} style={styles.reward}/>
